        ...
        <div class="sidebar__nav-head">
          ...
          <div class="sidebar__nav-icons">
            <NuxtLink
              ...
              aria-label="Inbox"
              @click="emit('close')"
              :class="['sidebar__settings-btn', { 'active': route.name === 'inbox' }]">
              <svg
                width="16"
                height="16"
                viewBox="0 0 24 24"
...
import { isAdmin, logoutUser, userEmail, userRole, API_BASE } from '~/composables/appState'
...
async function fetchUnreadCount() {
  try {
    if (!userEmail.value) return
    const response = await fetch(`${API_BASE}/api/email/unread-count?email=${userEmail.value}`)
